namespace WarehouseApi.FuzzyText
{
    ...
    public class FuzzyComparer
    {
        ...
        public bool Equals(string a, string b, Level level = Level.Fuzzy, Options options = Options.None)
        {
            ...
            return (Compare(a, b, options) * 8) <= (a.Length + b.Length) * ((int)level);
        }
        ...
        /// <param name="options"></param>
        /// <returns></returns>
        public int Compare(string a, string b, Options options = 0)
        {
            ...
            return Compare(ignoreCase ? a.ToLower() : a, ignoreCase ? b.ToLower() : b, 0, 0, SwapCost, TypoCost_similar, TypoCost_different, DuplicateCost, InsertCost, LengthCostBase, ExtraLengthCost);
        }
        ...
        /// <param name="SwapCost"></param>
        /// <param name="TypoCost_similar"></param>
        /// <param name="TypoCost_different"></param>
        /// <param name="DuplicateCost"></param>
        /// <param name="InsertCost"></param>
        /// <param name="LengthCostBase"></param>
        /// <param name="ExtraLengthCost"></param>
        /// <returns></returns> string, in order to find the
        /// 
        private int Compare(string a, string b, int ida, int idb, int SwapCost, int TypoCost_similar, int TypoCost_different, int DuplicateCost, int InsertCost, int LengthCostBase, int ExtraLengthCost,int maxAlength=-1/*Only search until this point in string a or b*/,int maxBlength=-1)
        {
            ...
            for (; ida < maxAlength || idb < maxBlength; ida++, idb++)
            {
                ...
                if (ida >= maxAlength)
                {
                    return LengthCostBase + ExtraLengthCost * (maxBlength - idb);
                }
                else if (idb >= maxBlength)
                {
                    return LengthCostBase + ExtraLengthCost * (maxAlength - ida);
                }
                else
                {
                    char A = a[ida];
                    ...
                    if (A != B)
                    {
                        ...
                        //Let us start with what the difference score would be, if the difference is a single char typo, but do check if it is a common typo (like 0 instead of the letter O) or an uncommon one
                        //The conditional statement first checks if the CommonTypos dictionary even has an entry for the one char, and if yes, if one of the known common typos is B

                        int minCost = Compare(a, b, ida + 1, idb + 1, SwapCost, TypoCost_similar, TypoCost_different, DuplicateCost, InsertCost, LengthCostBase, ExtraLengthCost,maxAlength,maxBlength) + ((CommonTypos.ContainsKey(A) && CommonTypos[A].Contains(B)) ? TypoCost_similar : TypoCost_different);
                        ...
                        if (nextA == B && nextB == A)
                            minCost = Math.Min(minCost, Compare(a, b, ida + 2, idb + 2, SwapCost, TypoCost_similar, TypoCost_different, DuplicateCost, InsertCost, LengthCostBase, ExtraLengthCost, maxAlength, maxBlength) + SwapCost);
                        ...
                        if (ida + 1 < maxAlength )//If the id of a is not outside range, check if we have inserted a single char in a, the conditional uses dublicate cost if the previous thing in a was the same as this (for example a=teext and b=text)
                            minCost = Math.Min(minCost, Compare(a, b, ida + 1, idb, SwapCost, TypoCost_similar, TypoCost_different, DuplicateCost, InsertCost, LengthCostBase, ExtraLengthCost, maxAlength, maxBlength) + (ida>0 && a[ida-1]==A? DuplicateCost : InsertCost));
                        if (idb + 1 < maxBlength)//Now check insertions in b
                            minCost = Math.Min(minCost, Compare(a, b, ida, idb + 1, SwapCost, TypoCost_similar, TypoCost_different, DuplicateCost, InsertCost, LengthCostBase, ExtraLengthCost, maxAlength, maxBlength)+ (idb>0 && b[idb-1]==B? DuplicateCost : InsertCost));

                        return minCost;
                    }
                }
            }
            return 0;//If we get here, no differences were found
        }
        ...
        {
            return ContainScore(a, b, out bestmatch, options) * 8 <= ((int)level) * (a.Length);
        }
        /// <summary>
        /// Same as above, but doesn't return the best match
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        /// <param name="level"></param>
        ...
        {
            if (ContainScore(a, b, out string bestmatch, options) * 8 <= ((int)level) * (a.Length))
            {
                Console.WriteLine($"Found {bestmatch} in {b}");
                return true;
            }
            else
                return false;
        ...
        /// <param name="options"></param>
        /// <returns></returns>
        public int ContainScore(string a, string b, out string bestmatch, Options options = 0)
        {
            ...
            int bestScore = Compare(ignoreCase ? a.ToLower() : a, ignoreCase ? b.ToLower() : b, 0, 0, SwapCost, TypoCost_similar, TypoCost_different, DuplicateCost, InsertCost, LengthCostBase, ExtraLengthCost, a.Length, a.Length); ;
            ...
            for (int start_idb = 1; start_idb + a.Length <= b.Length && bestScore != 0/*Quit when a perfect match is found*/; ++start_idb)
            {
                ...
                int thisScore = Compare(ignoreCase ? a.ToLower() : a, ignoreCase ? b.ToLower() : b, 0, start_idb, SwapCost, TypoCost_similar, TypoCost_different, DuplicateCost, InsertCost, LengthCostBase, ExtraLengthCost, a.Length, a.Length + start_idb);
